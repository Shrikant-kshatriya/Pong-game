...
class Ball{
    ...
    update(){
        ...
        // paddle2 Ball collision
        if(rightSide >= paddle2.position.x && bottomSide >= paddle2.position.y && topSide <= paddle2.position.y + paddle2.height)this.velocity.x = -this.velocity.x;
        
        // paddle1 Ball collision
        if(leftSide <= paddle1.position.x + paddle1.width && bottomSide >= paddle1.position.y && topSide <= paddle1.position.y + paddle1.height)this.velocity.x = -this.velocity.x;
        
        if(bottomSide >= canvas.height || topSide <= 0)this.velocity.y = -this.velocity.y;
...
const ball = new Ball({x : canvas.width / 2, y : canvas.height / 2});
